...
class CentresDataset(Dataset):
	...
	def __getitem__(self, index):
		...
		if path.split('.')[-1] == 'pkl':
			...
			data_mov = data["in"]+np.random.normal(0, 1, size=data_size)*0.05*np.ptp(data["in"])
			IN = add_gradients(data_mov)
...
def add_gradients(data):
	data_size = data.shape
	#make gradients
	gradT,gradX,gradY = np.mgrid[0:data_size[0],0:data_size[1],0:data_size[2]]\
			/np.max(data_size) #normalized
	data_mov = (data-np.min(data))/(np.max(data)-np.min(data))*get_mask(numpy=True)
	return np.array([data_mov,gradT,gradX,gradY])
